...
try:
    SQL_QUERY = SQLQueries.insert_data(table_name)
    cursor.execute(fr'USE {activ_db_name};')
    for data in all_data:
        cursor.execute(SQLQueries.insert_data(table_name, '(id, Название, Тип (Овощ или фрукт), Цвет, Калорийность, Краткое описание)', data))
except pyodbc.ProgrammingError as ex:
    print(ex)
